...
$(document).ready(function(){
    ...
    $(".side ul li").hover(function(){
        $(this).find(".sidebox").css({"background":"#AC1011","width":"200px","paddingLeft":"20px","transform":"rotate(360deg)"})    
    },function(){
        $(this).find(".sidebox").css({"background":" #AC1011","width":"50px","paddingLeft":"0px","transform":"rotate(0deg)"})   
    });
...
$(document).ready(function(){
//列表页，当前分类的当前项样式。
	var headerA = $('.sub-nav ul').find('a');
	//var headerAflag = headerA[0];
	var curHref = window.location.search.substr(1);
	//$(headerAflag).addClass('hov');
	headerA.each(function() {
		var eachHref = $(this).attr('href');
		eachHref = eachHref.substring(eachHref.indexOf("http://") + 7, eachHref.length);
		eachHref = eachHref.substring(eachHref.indexOf("?") + 1, eachHref.length);
		if (eachHref == curHref) {
			$(this).addClass('hov');
		} else {
